...
pub mod exports {
	...
	pub use super::{
			...
			encode_hex_build, encode_hex_extend,
			decode_hex_build, decode_hex_extend,
...
pub fn encode_hex_extend (bytes : &Value, buffer : &Value) -> (Outcome<Value>) {
	return encode_extend_0 (&ext::data_encoding::HEXLOWER, bytes, buffer);
}
...
pub fn decode_hex_extend (string : &Value, buffer : &Value) -> (Outcome<Value>) {
	return decode_extend_0 (&ext::data_encoding::HEXLOWER_PERMISSIVE, string, buffer);
}
...
fn encode_build_0 (encoding : &ext::data_encoding::Encoding, bytes : &Value) -> (Outcome<Value>) {
	...
	try! (encode_0 (encoding, bytes, &mut buffer));
	succeed! (string_new (buffer));
}


#[ cfg_attr ( feature = "vonuvoli_inline", inline ) ]
fn encode_extend_0 (encoding : &ext::data_encoding::Encoding, bytes : &Value, buffer : &Value) -> (Outcome<Value>) {
	let bytes = try_as_bytes_ref! (bytes);
	let bytes = bytes.bytes_as_slice ();
...
#[ cfg_attr ( feature = "vonuvoli_inline", inline ) ]
fn encode_0 (encoding : &ext::data_encoding::Encoding, bytes : &[u8], buffer : &mut StdString) -> (Outcome<()>) {
...
fn decode_build_0 (encoding : &ext::data_encoding::Encoding, string : &Value) -> (Outcome<Value>) {
	...
	try! (decode_0 (encoding, string, &mut buffer));
	succeed! (bytes_new (buffer));
}


#[ cfg_attr ( feature = "vonuvoli_inline", inline ) ]
fn decode_extend_0 (encoding : &ext::data_encoding::Encoding, string : &Value, buffer : &Value) -> (Outcome<Value>) {
	let string = try_as_string_ref! (string);
	let string = string.string_as_bytes ();
...
#[ cfg_attr ( feature = "vonuvoli_inline", inline ) ]
fn decode_0 (encoding : &ext::data_encoding::Encoding, string : &[u8], buffer : &mut StdVec<u8>) -> (Outcome<()>) {
